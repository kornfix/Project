...
//        setCampaignControllerAllCategories(model);
    ...
    public String addCampaign(@Valid @ModelAttribute("campaign") CampaignData campaignData, BindingResult result,
        ...
        if(!result.hasErrors()) {
            checkDates(campaignData, result);
        }
        if (result.hasErrors()) {
            return "saveCampaign";
        }
...
//            setCampaignControllerAllCategories(model);
    ...
    public void checkDates(CampaignData campaignData, BindingResult result) {
        if (campaignData.getCampaignStartDate().getTime() > campaignData.getCampaignEndDate().getTime()) {
            result.addError(new ObjectError("globalError", "Data zakończenia nie może być mniejsza od daty rozpoczęcia"));
        }
    }
}
